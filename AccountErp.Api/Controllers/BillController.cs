...
namespace AccountErp.Api.Controllers
{
   ...
   // [Authorize]
    ...
    public class BillController : ControllerBase
    {
        ...
        [HttpPost]
        [Route("getall")]
        public async Task<IActionResult> GetAllPaginationASync(int PageSize, int Page,int VendorId,string filterKey,DateTime? BillDate,int quantity)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState.GetErrorList());
            ...
            var data = await _manager.GetAllPagination(PageSize, Page, VendorId,filterKey,BillDate,quantity);
