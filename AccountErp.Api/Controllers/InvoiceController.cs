...
namespace AccountErp.Api.Controllers
{
   ...
   // [Authorize]
    ...
    public class InvoiceController : ControllerBase
    {
        ...
        public async Task<IActionResult> Add([FromBody]InvoiceAddModel model)
        {
            ...
            try
            {
                ...
                { 
                 ...
                 //   await _invoiceManager.AddInvoiceServiceTag(model.InvoiceServiceTag);
        ...
        [HttpPost]
        [Route("add-invoiceServiceTag")]
        public async Task<IActionResult> AddInvoiceServiceTag([FromBody] List<AddInvoiceServiceTagModel> model)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState.GetErrorList());
            ...
            try
            {
                ...
                if (model != null)
                {
                    await _invoiceManager.AddInvoiceServiceTag(model);
                    response.StatusCode = 200;
                    response.Message = Constants.added;
                    return Ok(response);
                }
        ...
        [AllowAnonymous]
        [Route("get-detail/{id}")]
        public async Task<IActionResult> GetDetail(int id,Constants.InvoiceValue InvoiceValue)
        {
            var invoice = await _invoiceManager.GetDetailAsync(id,InvoiceValue);
        ...
        public async Task<IActionResult> GetDetailAsyncforpyment(int id, Constants.InvoiceValue InvoiceValue )
        {
            var invoice = await _invoiceManager.GetDetailAsyncforpyment(id,InvoiceValue);
        ...
        [HttpPost]
        [Route("getAll")]
        public async Task<IActionResult> GetAllAsync(int PageSize,int Page,string filterKey, Constants.InvoiceValue InvoiceValue)
        {
            ...
            var data = await _invoiceManager.GetAllAsync(PageSize, Page,filterKey,InvoiceValue); 
        ...
        {
            var invoice = await _invoiceManager.GetDetailAsync(model.Id,model.InvoiceValue);
        ...
        public async Task<IActionResult> GetInvoiceNumber(Constants.InvoiceValue InvoiceValue)
        {
            return Ok(await _invoiceManager.GetInvoiceNumber( InvoiceValue));
        }
