    ...
    public class BillManager : IBillManager
    {
      ...
      .Select(g => new { GroupId = g.Key, Values = g.ToList() })).ToList();
       ...
       .Select(g => new { GroupId = g.Key, Values = g.ToList() })).ToList();
        ...
        public async Task<(List<BillDetailDto>, int count)> GetAllPagination(int PageSize,int Page,int vendorId,string filterKey, DateTime? BillDate,int quantity)
        {
            List<BillDetailDto> r = new List<BillDetailDto>();
          
            var data =  await _repository.GetAllPagination(PageSize, Page,vendorId,filterKey,BillDate);
           
            foreach (var item in data.Item1)
            {
                item.Items = await _repository.GetBillItemById(item.Id);
            }
        ...
        {
            return await _repository.PurchaseReportAsync(vendorId,From,To);
        }
