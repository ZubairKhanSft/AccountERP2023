...
namespace AccountErp.Managers
{
    public class InvoiceManager : IInvoiceManager
    {
        ...
        public async Task<int> AddAsync(InvoiceAddModel model)
        {
            ...
            var count = await _invoiceRepository.getCount(model.InvoiceValue);
...
*/
       ...
       .Select(g => new { GroupId = g.Key, Values = g.ToList() })).ToList();
        ...
        public async Task EditAsync(InvoiceEditModel model)
        {
         ...
         /*   await _invoiceRepository.DeleteInvoiceServiceTags(model.Id);
            await _invoiceRepository.AddInvoiceServiceTag(InvoiceFactory.CreateInvoiceServiceTag(model.InvoiceServiceTag));
*/
       ...
       .Select(g => new { GroupId = g.Key, Values = g.ToList() })).ToList();
        ...
        {
           var data =  await _invoiceRepository.GetDetailAsync(id,InvoiceValue);

            data.InvoiceServiceTag = await _invoiceRepository.GetInvoiceServiceTagByInvoiceId(data.Id);
            return data;
        }
        public async Task<InvoiceDetailDto> GetDetailAsyncforpyment(int id, Constants.InvoiceValue InvoiceValue)
        {
            var data =  await _invoiceRepository.GetDetailAsyncforpyment(id,InvoiceValue);
            data.InvoiceServiceTag = await _invoiceRepository.GetInvoiceServiceTagByInvoiceId(data.Id);
            return data;
        }
        ...
        {
            var data = await _invoiceRepository.GetAllAsync(PageSize,Page,filterKey,InvoiceValue);
            return data;
        }
        ...
        public async Task<int> GetInvoiceNumber(Constants.InvoiceValue InvoiceValue)
        {
            var count = await _invoiceRepository.getCount(InvoiceValue);
            return (count + 1);
        }
        ...
        public async Task AddInvoiceServiceTag(List<AddInvoiceServiceTagModel> model)
        {
            await _invoiceRepository.AddInvoiceServiceTag(InvoiceFactory.CreateInvoiceServiceTag(model));
        }
