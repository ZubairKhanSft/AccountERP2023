...
namespace AccountErp.DataLayer.Repositories
{
    public class InvoiceRepository : IInvoiceRepository
    {
        ...
        public async Task<InvoiceDetailDto> GetDetailAsync(int id,Constants.InvoiceValue InvoiceValue)
        {
            ...
            var invoice = await (from i in _dataContext.Invoices
                                 ...
                                 where i.Id == id && i.Status != Constants.InvoiceStatus.Deleted && i.InvoiceValue == InvoiceValue
                                 select new InvoiceDetailDto
                                 {
                                     ...
                                     ShippingTerm = i.ShippingTerm,
                                     PaymentTerm = i.PaymentTerm,

                                     ShippingCompanyName = i.ShippingCompanyName,
                                     RecipientName = i.RecipientName,
                                     ShippingCountry = i.ShippingCountry,
                                     ShippingCity = i.ShippingCity,
                                     ShippingState = i.ShippingState,
                                     ShippingTRN = i.ShippingTRN,
                                     ShippingPostalCode = i.ShippingPostalCode,
        ...
        public async Task<List<InvoiceServiceTagDetailDto>> GetInvoiceServiceTagByInvoiceId(int invoiceId)
        {
            return await (from s in _dataContext.InvoiceTag
                          where s.InvoiceId == invoiceId
                          select new InvoiceServiceTagDetailDto
                          {
                              Id = s.id,
        ...
        public async Task<(List<InvoiceDetailDto> data, int count)> GetAllAsync(int PageSize,int Page,string FilterKey, Constants.InvoiceValue InvoiceValue)
        {
            var response = new List<InvoiceDetailDto>();
            int count=0;
            count = await _dataContext.Invoices.Where(s => s.Status != Constants.InvoiceStatus.Deleted && s.InvoiceValue == InvoiceValue).CountAsync();
...
*/
    ...
    */
                                   ...
                                   /*join add in _dataContext.ShippingAddress on c.Id equals add.CustomerId into shipadd
                                   from ship in shipadd.DefaultIfEmpty()*/
                                   where i.Status != Constants.InvoiceStatus.Deleted && i.InvoiceValue == InvoiceValue
                                    && (FilterKey == null
...
*/
        ...
        public async Task<InvoiceDetailDto> GetDetailAsyncforpyment(int id, Constants.InvoiceValue InvoiceValue)
        {
            var invoice = await (from i in _dataContext.Invoices
                                 join c in _dataContext.Customers
                                 on i.CustomerId equals c.Id
        ...
        public async Task<int> getCount( Constants.InvoiceValue InvoiceValue)
        {
            int count = await _dataContext.Invoices.CountAsync(s=> s.InvoiceValue == InvoiceValue);
            return count;
        }
        ...
        public async Task AddInvoiceServiceTag(List<InvoiceTag> entity)
        {
            await _dataContext.InvoiceTag.AddRangeAsync(entity);
            await _dataContext.SaveChangesAsync();
            ...
            /*foreach (var item in entity)
            {
                try
                {
                    await _dataContext.InvoiceServiceTag.AddRangeAsync(item);
                    await _dataContext.SaveChangesAsync();
                }
                catch(Exception ex)
                {
        ...
        public async Task DeleteInvoiceServiceTags(int invoiceId)
        {
            var data = await _dataContext.InvoiceServiceTag.Where(s => s.InvoiceId == invoiceId).ToListAsync();
            _dataContext.RemoveRange(data);
            _dataContext.SaveChanges();
        }
